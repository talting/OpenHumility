...
public class Notification {
   private final Notification.Type t;
   ...
   public Notification(String message, Notification.Type type) {
      super();
   ...
   public void draw(double getY, double lastY) {
      ...
      this.imageWidth = 11;
      this.lastY = lastY;
      this.animationX = this.getAnimationState(this.animationX, this.isFinished() ? this.width : 0.0D, Math.max(this.isFinished() ? 200.0D : 30.0D, Math.abs(this.animationX - (this.isFinished() ? this.width : 0.0D)) * 20.0D) * 0.3D);
      if (this.posY == -1.0D) {
         this.posY = getY;
      } else {
         this.posY = this.getAnimationState(this.posY, getY, 200.0D);
      }
      ...
      if (this.message.contains(" Enabled")) {
         Hanabi.INSTANCE.fontManager.comfortaa15.drawString(this.message.replace(" Enabled", ""), (float)(x1 + 19), (float)((double)y1 + this.height / 4.0D), -1);
         Hanabi.INSTANCE.fontManager.comfortaa15.drawString(" Enabled", (float)(x1 + 20 + Hanabi.INSTANCE.fontManager.comfortaa15.getStringWidth(this.message.replace(" Enabled", ""))), (float)((double)y1 + this.height / 4.0D), Colors.GREY.c);
      } else if (this.message.contains(" Disabled")) {
         Hanabi.INSTANCE.fontManager.comfortaa15.drawString(this.message.replace(" Disabled", ""), (float)(x1 + 19), (float)((double)y1 + this.height / 4.0D), -1);
         Hanabi.INSTANCE.fontManager.comfortaa15.drawString(" Disabled", (float)(x1 + 20 + Hanabi.INSTANCE.fontManager.comfortaa15.getStringWidth(this.message.replace(" Disabled", ""))), (float)((double)y1 + this.height / 4.0D), Colors.GREY.c);
      } else {
         Hanabi.INSTANCE.fontManager.comfortaa15.drawString(this.message, (float)(x1 + 20), (float)((double)y1 + this.height / 4.0D), -1);
      }
