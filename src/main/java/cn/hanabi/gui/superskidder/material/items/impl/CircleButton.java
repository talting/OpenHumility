...
public class CircleButton extends SButton {
   ...
   public void draw(double mouseX, double mouseY, int mouseButton) {
      for(Shadow s : this.ss) {
         GL11.glEnable(3089);
         RenderUtil.doGlScissor((int)this.x, (int)this.y, (int)this.width, (int)this.height);
         RenderUtil.drawFilledCircle((int)((double)((int)this.x) + this.width / 2.0D), (int)((double)((int)this.y) + this.height / 2.0D), (float)s.size, (new Color(255, 255, 255, (int)s.alpha)).getRGB());
         GL11.glDisable(3089);
      }
