   ...
   protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
      ...
      for(final Category mt : this.categories) {
         if (mt.show) {
            ...
            for(final Mod m : ModManager.getModules(mt.moduleType)) {
      ...
      for(final Tab t : tabs) {
         ...
         if (isHovered(t.x + swidth - 4.0F, windowY + 30.0F, t.x + swidth + 4.0F, windowY + 50.0F, (float)mouseX, (float)mouseY) && Mouse.isButtonDown(0)) {
            tabs2.add(t);
         }
