   ...
   public void renderOverlay() {
      ...
      if (MusicManager.INSTANCE.getCurrentTrack() != null && MusicManager.INSTANCE.getMediaPlayer() != null) {
         ...
         if (MusicManager.INSTANCE.circleLocations.containsKey(Long.valueOf(MusicManager.INSTANCE.getCurrentTrack().id))) {
            ...
            ResourceLocation icon = (ResourceLocation)MusicManager.INSTANCE.circleLocations.get(Long.valueOf(MusicManager.INSTANCE.getCurrentTrack().id));
         ...
         try {
            float currentProgress = (float)(MusicManager.INSTANCE.getMediaPlayer().getCurrentTime().toSeconds() / Math.max(1.0D, MusicManager.INSTANCE.getMediaPlayer().getStopTime().toSeconds())) * 100.0F;
            RenderUtil.drawArc((float)(18 + addonX), (float)(19 + addonY), 14.0D, Colors.WHITE.c, 0, 360.0D, 4);
            RenderUtil.drawArc((float)(18 + addonX), (float)(19 + addonY), 14.0D, Colors.BLUE.c, 180, (double)(180.0F + currentProgress * 3.6F), 4);
      ...
      if (MusicManager.INSTANCE.lyric) {
         ...
         lyricFont.drawCenteredString(MusicManager.INSTANCE.lrcCur.contains("_EMPTY_") ? "等待中......." : MusicManager.INSTANCE.lrcCur, (float)sr.getScaledWidth() / 2.0F - 0.5F, (float)(sr.getScaledHeight() - 140 - 80 + addonYlyr), -16732281);
         lyricFont.drawCenteredString(MusicManager.INSTANCE.tlrcCur.contains("_EMPTY_") ? "Waiting......." : MusicManager.INSTANCE.tlrcCur, (float)sr.getScaledWidth() / 2.0F, (float)(sr.getScaledHeight() - 125) + 0.5F - 80.0F + (float)addonYlyr, col);
      ...
      if (MusicManager.showMsg) {
         ...
         RenderUtil.drawArc((float)sr.getScaledWidth() - this.animation - 31.0F + 50.0F, 22.0F, 14.0D, Colors.WHITE.c, 0, 360.0D, 2);
         ...
         if (this.timer.isDelayComplete(5000L)) {
            this.animation = (float)RenderUtil.getAnimationStateSmooth(0.0D, (double)this.animation, (double)(10.0F / (float)Minecraft.getDebugFPS()));
