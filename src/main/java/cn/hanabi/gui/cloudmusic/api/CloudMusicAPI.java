...
public enum CloudMusicAPI {
   ...
   public Object[] httpRequest(String url, String data, CookieStore cookie, CloudMusicAPI.RequestType type) throws Exception {
      ...
      CookieStore cookieStore = (CookieStore)(cookie == null ? new BasicCookieStore() : cookie);
      cookieStore.clear();
      if (cookie == null) {
      ...
      HttpClient httpClient = HttpClients.custom().setDefaultRequestConfig(config).setDefaultCookieStore(cookieStore).build();
      String resp = "";
      switch(type) {
      case POST:
   ...
   public void downloadFile(String url, String filepath) {
      try {
         ...
         int var19;
         for(int ch = 0; (var19 = is.read(buffer)) != -1; MusicManager.INSTANCE.downloadProgress = (float)(progress / unit)) {
            fileout.write(buffer, 0, var19);
            progress += (long)var19;
         }
