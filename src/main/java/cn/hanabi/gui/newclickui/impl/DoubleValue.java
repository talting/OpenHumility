...
public class DoubleValue {
   ...
   public void draw(float x, float y, float mouseX, float mouseY) {
      ...
      double max = ((Double)this.values.getValueMax()).doubleValue();
      double min = ((Double)this.values.getValueMin()).doubleValue();
      double valn = ((Double)this.values.getValueState()).doubleValue();
      int longValue = 90;
      ...
      if (ClickUI.isHover(mouseX, mouseY, x + 120.0F, y + 2.0F, x + 230.0F, y + this.length - 2.0F)) {
         ...
         font.drawCenteredString(((Double)this.values.getValueState()).doubleValue() + "", (float)((double)(x + 125.0F) + v + 4.0D), y, (new Color(255, 255, 255, 255)).getRGB());
      }
   ...
   public void handleMouseinRender(float mouseX, float mouseY, int key) {
      if (ClickUI.isHover(mouseX, mouseY, this.x + 120.0F, this.y + 2.0F, this.x + 230.0F, this.y + this.length - 2.0F) && Mouse.isButtonDown(0) && this.premouse) {
         ...
         double perc = valAbs / ((double)longValue * Math.max(Math.min(valn / max, 0.0D), 1.0D));
         perc = Math.min(Math.max(0.0D, perc), 1.0D);
         double valRel = (max - min) * perc;
         double val = min + valRel;
         val = (double)Math.round(val * (1.0D / inc)) / (1.0D / inc);
         this.values.setValueState(Double.valueOf(val));
      }
