...
public class BoolValue {
   ...
   public BoolValue(Value values) {
      ...
      this.anima.setXY(((Boolean)values.getValueState()).booleanValue() ? 10.0F : 0.0F, ((Boolean)values.getValueState()).booleanValue() ? 0.0F : 255.0F);
   }

   public void draw(float x, float y, float mouseX, float mouseY) {
      this.x = x;
      this.y = y;
      this.anima.interpolate(((Boolean)this.values.getValueState()).booleanValue() ? 10.0F : 0.0F, ((Boolean)this.values.getValueState()).booleanValue() ? 255.0F : 0.0F, 0.25F);
      this.length = 30.0F;
   ...
   public void handleMouse(float mouseX, float mouseY, int key) {
      if (ClickUI.isHover(mouseX, mouseY, this.x + 205.0F, this.y + 7.0F, this.x + 225.0F, this.y + 17.0F) && key == 0) {
         this.values.setValueState(Boolean.valueOf(!((Boolean)this.values.getValueState()).booleanValue()));
      }
