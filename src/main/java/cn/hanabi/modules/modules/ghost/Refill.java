...
public class Refill extends Mod {
   public Value delay = new Value("Refill", "Delay", 500.0D, 0.0D, 2000.0D, 50.0D);
   ...
   public void onUpdate(EventPreMotion event) {
      if (((Boolean)this.Soup.getValue()).booleanValue()) {
         this.value = Items.mushroom_stew;
      } else if (((Boolean)this.Pot.getValue()).booleanValue()) {
   ...
   private void refill() {
      if ((!((Boolean)this.onInv.getValue()).booleanValue() || mc.currentScreen instanceof GuiInventory) && !isHotbarFull() && this.time.isDelayComplete((Double)this.delay.getValue())) {
         refill(this.value);
         this.time.reset();
      }
