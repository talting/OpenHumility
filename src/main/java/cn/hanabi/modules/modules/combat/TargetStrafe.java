...
public class TargetStrafe extends Mod {
   ...
   public static Value range = new Value("TargetStrafe", "Strafe Range", 2.0D, 0.5D, 4.5D, 0.1D);
   ...
   public static boolean canStrafe() {
      boolean press = !((Boolean)targetkey.getValue()).booleanValue() || mc.gameSettings.keyBindJump.isKeyDown();
   ...
   private void onRender3D(EventRender event) {
      if (canStrafe() && ((Boolean)render.getValue()).booleanValue()) {
         EntityLivingBase target = KillAura.target;
         this.esp(target, event.getPartialTicks(), ((Double)range.getValue()).doubleValue());
      }
   ...
   public void esp(Entity entity, float partialTicks, double rad) {
      ...
      for(double il = 0.0D; il < Double.MIN_VALUE; il += Double.MIN_VALUE) {
         ...
         for(int i = 0; i <= 90; ++i) {
            ...
            GL11.glVertex3d(x + rad * Math.cos((double)i * 6.283185307179586D / (double)points), y + il, z + rad * Math.sin((double)i * 6.283185307179586D / (double)points));
         }
   ...
   public boolean isStrafing(EventMove event, EntityLivingBase target, double moveSpeed) {
      boolean pressingSpace = !((Boolean)targetkey.getValue()).booleanValue() || Keyboard.isKeyDown(mc.gameSettings.keyBindJump.getKeyCode());
      if (this.isEnabled() && target != null && moveSpeed != 0.0D && pressingSpace) {
         ...
         if (mc.thePlayer.isCollidedHorizontally || aroundVoid && ((Boolean)voidCheck.getValue()).booleanValue() || behindTarget && ((Boolean)behindValue.getValue()).booleanValue()) {
            this.strafe *= -1;
         }

         float targetStrafe = ((Boolean)change.getValue()).booleanValue() && mc.thePlayer.moveStrafing != 0.0F ? mc.thePlayer.moveStrafing * (float)this.strafe : (float)this.strafe;
         if (PlayerUtil.isBlockUnder()) {
            targetStrafe = 0.0F;
         }

         double rotAssist = 45.0D / this.getEnemyDistance(target);
