...
public class AutoSword extends Mod {
   ...
   public Value slot = new Value("AutoSword", "Slot", 1.0D, 1.0D, 9.0D, 1.0D);
   ...
   private float getSharpnessLevel(ItemStack stack) {
      ...
      damage = damage + (float)EnchantmentHelper.getEnchantmentLevel(Enchantment.sharpness.effectId, stack) * 1.25F;
      damage = damage + (float)EnchantmentHelper.getEnchantmentLevel(Enchantment.fireAspect.effectId, stack) * 0.01F;
      return damage;
   }
}
