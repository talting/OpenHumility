...
public class AimBot extends Mod {
   ...
   public static Value range = new Value("AimBot", "Range", 100.0D, 1.0D, 500.0D, 5.0D);
   public static Value deviation = new Value("AimBot", "Pre-Attack", 1.5D, 0.0D, 10.0D, 0.1D);
   private Vec3 aimed;
   ...
   private void onUpdatePre(EventPreMotion event) {
      List<Entity> targets = (List)WorldUtil.getLivingEntities().stream().filter(this::isValid).sorted(Comparator.comparing((e) -> {
      ...
      if (list.size() > 0) {
         this.aimed = this.getFixedLocation((EntityLivingBase)list.get(0), ((Double)deviation.getValue()).floatValue(), ((Boolean)headshot.getValue()).booleanValue());
   ...
   private void onRender3D(EventRender event) {
      if (this.aimed != null) {
         ...
         RenderUtil.drawBlockESP(posX - 0.5D, posY - 0.5D, posZ - 0.5D, (new Color(255, 0, 0, 100)).getRGB(), (new Color(16771328)).getRGB(), 0.4F, 0.1F);
      }
   }
   ...
   private Vec3 getFixedLocation(EntityLivingBase entity, float velocity, boolean head) {
      ...
      double y = entity.posY + (entity.posY - entity.lastTickPosY) * (double)velocity * 0.3D + (head ? (double)entity.getEyeHeight() : 1.0D);
      double z = entity.posZ + (entity.posZ - entity.lastTickPosZ) * (double)velocity;
      return new Vec3(x, y, z);
   }

   private boolean isValid(EntityLivingBase entity) {
      if (entity instanceof EntityZombie || entity instanceof EntitySilverfish || entity instanceof EntityWither || entity instanceof EntityGhast || entity instanceof EntitySpider || entity instanceof EntityGiantZombie || entity instanceof EntitySkeleton || entity instanceof EntityGolem || entity instanceof EntityEndermite || entity instanceof EntityWitch || entity instanceof EntityBlaze || entity instanceof EntitySlime || entity instanceof EntityCreeper || entity instanceof EntityWolf || entity instanceof EntityPlayer && ((Boolean)players.getValue()).booleanValue()) {
         if (!entity.isDead && entity.getHealth() > 0.0F) {
            return (double)mc.thePlayer.getDistanceToEntity(entity) > ((Double)range.getValue()).doubleValue() ? false : PlayerUtil.canEntityBeSeenFixed(entity);
         } else {
            return false;
         }
      } else {
