...
public class Nuker extends Mod {
   ...
   private final Value reach = new Value("Nuker", "Reach", 6.0D, 1.0D, 6.0D, 0.1D);
   private final Value delay = new Value("Nuker", "Delay", 120.0D, 0.0D, 1000.0D, 10.0D);
   ...
   public void onPre(EventPreMotion event) {
      Iterator positions = BlockPos.getAllInBox(mc.thePlayer.getPosition().subtract(new Vec3i(((Double)this.reach.getValueState()).doubleValue(), ((Double)this.reach.getValueState()).doubleValue(), ((Double)this.reach.getValueState()).doubleValue())), mc.thePlayer.getPosition().add(new Vec3i(((Double)this.reach.getValueState()).doubleValue(), ((Double)this.reach.getValueState()).doubleValue(), ((Double)this.reach.getValueState()).doubleValue()))).iterator();
      ...
      if (bedPos != null) {
         ...
         if (this.timer.isDelayComplete((Double)this.delay.getValueState())) {
            if (((Boolean)this.instant.getValueState()).booleanValue()) {
   ...
   public float[] getRotations(BlockPos block, EnumFacing face) {
      ...
      double z = (double)block.getZ() + 0.5D - mc.thePlayer.posZ;
      double d1 = mc.thePlayer.posY + (double)mc.thePlayer.getEyeHeight() - ((double)block.getY() + 0.5D);
