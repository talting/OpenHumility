...
public class ChestStealer extends Mod {
   ...
   public static Value onlychest = new Value("ChestStealer", "Only Chest", Boolean.TRUE);
   private final Value delay = new Value("ChestStealer", "Delay", 30.0D, 0.0D, 1000.0D, 10.0D);
   private final Value trash = new Value("ChestStealer", "Pick Trash", Boolean.TRUE);
   private final Value tools = new Value("ChestStealer", "Tools", Boolean.TRUE);
   ...
   public void onUpdate(EventUpdate event) {
      ...
      if (isChest || !((Boolean)onlychest.getValueState()).booleanValue()) {
         if (mc.thePlayer.openContainer != null && mc.thePlayer.openContainer instanceof ContainerChest) {
            ...
            for(int i = 0; i < c.getLowerChestInventory().getSizeInventory(); ++i) {
               if (c.getLowerChestInventory().getStackInSlot(i) != null && this.time.isDelayComplete(Double.valueOf(((Double)this.delay.getValueState()).doubleValue() + (double)(new Random()).nextInt(100))) && (this.itemIsUseful(c, i) || ((Boolean)this.trash.getValueState()).booleanValue()) && (new Random()).nextInt(100) <= 80) {
   ...
   private boolean isChestEmpty(ContainerChest c) {
      for(int i = 0; i < c.getLowerChestInventory().getSizeInventory(); ++i) {
         if (c.getLowerChestInventory().getStackInSlot(i) != null && (this.itemIsUseful(c, i) || ((Boolean)this.trash.getValueState()).booleanValue())) {
            return false;
         }
      }
   ...
   private boolean itemIsUseful(ContainerChest c, int i) {
      ...
      if ((item instanceof ItemAxe || item instanceof ItemPickaxe) && ((Boolean)this.tools.getValueState()).booleanValue()) {
         return true;
   ...
   private boolean isBestSword(ContainerChest c, ItemStack item) {
      ...
      for(int i = 0; i < 45; ++i) {
         if (mc.thePlayer.inventoryContainer.getSlot(i).getHasStack()) {
            float tempdamage = this.getSwordDamage(mc.thePlayer.inventoryContainer.getSlot(i).getStack());
      ...
      for(int i = 0; i < c.getLowerChestInventory().getSizeInventory(); ++i) {
         if (c.getLowerChestInventory().getStackInSlot(i) != null) {
            float tempdamage = this.getSwordDamage(c.getLowerChestInventory().getStackInSlot(i));
            if (tempdamage >= itemdamage2) {
               itemdamage2 = tempdamage;
            }
         }
