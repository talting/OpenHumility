...
public class Phase extends Mod {
   ...
   public void onMove(EventMove event) {
      if (isInsideBlock()) {
         if (mc.gameSettings.keyBindJump.isKeyDown()) {
            event.setY(mc.thePlayer.motionY += 0.09000000357627869D);
   ...
   public void onPost(EventPostMotion event) {
      ...
      if (mc.thePlayer.isCollidedHorizontally && !mc.thePlayer.isOnLadder() && mc.thePlayer.onGround) {
         ...
         mc.getNetHandler().getNetworkManager().sendPacket(new C04PacketPlayerPosition(posX + x, posY, posZ + z, true));
         mc.getNetHandler().getNetworkManager().sendPacket(new C04PacketPlayerPosition(posX, posY + 3.0D, posZ, true));
