...
public class UhcHelper extends Mod {
   ...
   public void OnUpdate(EventUpdate e) {
      ...
      if (((Boolean)this.sandbreak.getValueState()).booleanValue() && (sandblock == Block.getBlockById(12) || sandblock == Block.getBlockById(13))) {
         this.bestTool(mc.objectMouseOver.getBlockPos().getX(), mc.objectMouseOver.getBlockPos().getY(), mc.objectMouseOver.getBlockPos().getZ());
      ...
      if (((Boolean)this.lessPacket.getValueState()).booleanValue()) {
         mc.thePlayer.setGameType(GameType.SURVIVAL);
         mc.thePlayer.setGameType(GameType.CREATIVE);
      }

      if (((Boolean)this.autojump.getValue()).booleanValue() && PlayerUtil.isUnderBlock(mc.thePlayer) && PlayerUtil.isMoving2() && mc.thePlayer.onGround && Keyboard.isKeyDown(56)) {
         mc.thePlayer.jump();
      }
   ...
   public void onPre(EventPreMotion e) {
      if (((Boolean)this.autowater.getValue()).booleanValue() && mc.thePlayer.isBurning() && (this.getSlotWaterBucket() != -1 || this.reFill)) {
         e.setPitch(90.0F);
      }

      if (mc.gameSettings.keyBindSneak.isKeyDown() && ((Boolean)this.sneakily.getValue()).booleanValue()) {
   ...
   public void onPost(EventPostMotion post) {
      ...
      if (((Boolean)this.autowater.getValue()).booleanValue() && mc.thePlayer.isBurning()) {
         if (this.reFill) {
         ...
         if (this.getSlotWaterBucket() != -1 && this.timer.isDelayComplete(500L)) {
            ...
            BlockPos pos = new BlockPos(mc.thePlayer.posX, mc.thePlayer.posY - BlockUtils.getDistanceToFall() - 1.0D, mc.thePlayer.posZ);
            this.placeWater(pos);
   ...
   private void placeWater(BlockPos pos) {
      ...
      mc.playerController.onPlayerRightClick(mc.thePlayer, mc.theWorld, mc.thePlayer.inventory.getCurrentItem(), pos, EnumFacing.UP, new Vec3((double)pos.getX() + 0.5D, (double)pos.getY() + 1.0D, (double)pos.getZ() + 0.5D));
      if (heldItem != null) {
   ...
   public void onPacketReceive(EventPacket packetEvent) {
      if (((Boolean)this.noSandDamage.getValueState()).booleanValue() && packetEvent.packet instanceof C03PacketPlayer && this.isInsideBlock()) {
         packetEvent.setCancelled(true);
      }

      if (((Boolean)this.noWaterDamage.getValueState()).booleanValue() && packetEvent.packet instanceof C03PacketPlayer && PlayerUtil.isInWater() && Keyboard.isKeyDown(42)) {
         packetEvent.setCancelled(true);
      }

      if (((Boolean)this.LightningCheck.getValueState()).booleanValue() && packetEvent.packet instanceof S2CPacketSpawnGlobalEntity) {
   ...
   public void onEnable() {
      if (((Boolean)this.noWaterDamage.getValueState()).booleanValue()) {
