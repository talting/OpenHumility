...
public class Jesus extends Mod {
   ...
   public static Value motionY = new Value("Jesus", "Motion Y", 0.27D, 0.01D, 0.4D, 0.01D);
         ...
         for(int x = MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().minX); x < MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().maxX) + 1; ++x) {
            for(int z = MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().minZ); z < MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().maxZ) + 1; ++z) {
               ...
               if (block != null && block.getMaterial() != Material.air) {
                  ...
                  if (mc.theWorld.getBlockState(new BlockPos(x, y, z)).getProperties().get(BlockLiquid.LEVEL) instanceof Integer && ((Integer)((Comparable)mc.theWorld.getBlockState(new BlockPos(x, y, z)).getProperties().get(BlockLiquid.LEVEL))).intValue() > 1) {
                     return false;
                  }
   ...
   public void onBounding(BBSetEvent event) {
      if (mode.isCurrentMode("Solid") && event.getBlock() instanceof BlockLiquid && !mc.thePlayer.isInWater() && !mc.thePlayer.isInLava() && this.isPossible(event.pos)) {
         event.setBoundingBox((new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D)).contract(0.0D, 2.0E-12D, 0.0D).offset((double)event.getPos().getX(), (double)event.getPos().getY(), (double)event.getPos().getZ()));
      }
   ...
   public void onPacket(EventPreMotion event) {
      if (mode.isCurrentMode("Solid") && mc.thePlayer.onGround && !mc.thePlayer.isInWater() && BlockUtils.isOnLiquid() && !mc.thePlayer.isSneaking()) {
         event.y += mc.thePlayer.ticksExisted % 2 == 0 ? 2.0E-12D : 0.0D;
      }
      ...
      if (mode.isCurrentMode("BHop")) {
         if (BlockUtils.isInLiquid() && this.timer.isDelayComplete(200L) && !mc.gameSettings.keyBindSneak.isKeyDown() && !mc.thePlayer.isInWater()) {
            ...
            if (forward == 0.0F && strafe == 0.0F) {
               ...
               mc.thePlayer.motionX += (double)(strafe * var6 - forward * var5);
               mc.thePlayer.motionZ += (double)(forward * var6 + strafe * var5);
               this.timer.reset();
            }
   ...
   public boolean shouldGround(double n) {
      return n % 1.0D == 0.015625D || n % 1.0D == 0.0625D || n % 0.125D == 0.0D;
   }
   ...
   public void onBlockCollide(BBSetEvent event) {
      ...
      if (mode.isCurrentMode("Motion") && !ModManager.getModule("Speed").isEnabled() && !this.isInLiquid() && !mc.gameSettings.keyBindSneak.isKeyDown() && !mc.gameSettings.keyBindJump.isKeyDown()) {
         if (mc.theWorld.getBlockState(new BlockPos(x, y, z)).getProperties().get(BlockLiquid.LEVEL) instanceof Integer && ((Integer)((Comparable)mc.theWorld.getBlockState(new BlockPos(x, y, z)).getProperties().get(BlockLiquid.LEVEL))).intValue() > 2) {
            return;
         }

         Object v2 = mc.theWorld.getBlockState(event.getPos()).getProperties().get(BlockLiquid.LEVEL);
         if ((!(v2 instanceof Integer) || ((Integer)v2).intValue() <= 3) && mc.theWorld.getBlockState(event.getPos()).getBlock() instanceof BlockLiquid && !mc.thePlayer.isSneaking()) {
            event.setCancelled(true);
            event.setBoundingBox(new AxisAlignedBB((double)event.getPos().getX(), (double)event.getPos().getY(), (double)event.getPos().getZ(), (double)event.getPos().getX() + 0.98D, (double)event.getPos().getY() + 1.0D, (double)event.getPos().getZ() + 0.98D));
         }
      }
         ...
         for(int x = MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().minX); x < MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().maxX) + 1; ++x) {
            for(int z = MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().minZ); z < MathHelper.floor_double(mc.thePlayer.getEntityBoundingBox().maxZ) + 1; ++z) {
               ...
               if (block != null && !(block instanceof BlockAir)) {
                  if (block instanceof BlockLiquid && mc.theWorld.getBlockState(pos).getProperties().get(BlockLiquid.LEVEL) instanceof Integer && ((Integer)((Comparable)mc.theWorld.getBlockState(pos).getProperties().get(BlockLiquid.LEVEL))).intValue() > 1) {
                     return false;
                  }
   ...
   public void Motion(EventPreMotion em) {
      if (mc.thePlayer.fallDistance == 0.0F) {
         ...
         ++this.stage;
         if (this.stage == 1) {
            em.setY(em.y - ThreadLocalRandom.current().nextDouble(0.0155249999999D, 0.015625D));
         }

         if (this.stage == 2) {
            em.setY(em.y + ThreadLocalRandom.current().nextDouble(0.0148999999999D, 0.015D));
         }

         if (this.stage == 3) {
            em.setY(em.y + ThreadLocalRandom.current().nextDouble(0.0198999999999D, 0.02D));
         }
   ...
   public void flat() {
      mc.thePlayer.motionY = 0.11999998688698D;
   }
