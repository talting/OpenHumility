...
public class SpeedMine extends Mod {
   public static Value speed = new Value("SpeedMine", "Speed", 0.8D, 0.3D, 1.0D, 0.1D);
   public static Value Pspeed = new Value("SpeedMine", "PacketSpeed", 1.6D, 1.0D, 3.0D, 0.1D);
   ...
   public void onBreak(DamageBlockEvent event) {
      if (this.canBreak(event.getPos()) && ((Boolean)this.pot.getValueState()).booleanValue()) {
   ...
   public void onUpdate(EventUpdate event) {
      ...
      if (mc.playerController.extendedReach()) {
         ...
         if ((double)controller.getCurBlockDamageMP() >= 1.0D - ((Double)speed.getValueState()).doubleValue()) {
