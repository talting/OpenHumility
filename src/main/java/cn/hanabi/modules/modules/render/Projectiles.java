   ...
   public void onRender(EventRender event) {
      if (mc.thePlayer.inventory.getCurrentItem() != null) {
         ...
         if (item == 261 || item == 368 || item == 332 || item == 344) {
            double posX = player.lastTickPosX + (player.posX - player.lastTickPosX) * (double)Wrapper.getTimer().renderPartialTicks - Math.cos(Math.toRadians((double)player.rotationYaw)) * 0.1599999964237213D;
            double posY = player.lastTickPosY + (player.posY - player.lastTickPosY) * (double)Wrapper.getTimer().renderPartialTicks + (double)player.getEyeHeight() - 0.1D;
            double posZ = player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * (double)Wrapper.getTimer().renderPartialTicks - Math.sin(Math.toRadians((double)player.rotationYaw)) * 0.1599999964237213D;
            ...
            trajectoryX = trajectoryX / trajectory;
            trajectoryY = trajectoryY / trajectory;
            trajectoryZ = trajectoryZ / trajectory;
            if (stack.getItem() instanceof ItemBow) {
               ...
               trajectoryX = trajectoryX * (double)bowPower;
               trajectoryY = trajectoryY * (double)bowPower;
               trajectoryZ = trajectoryZ * (double)bowPower;
            } else {
               trajectoryX = trajectoryX * 1.5D;
               trajectoryY = trajectoryY * 1.5D;
               trajectoryZ = trajectoryZ * 1.5D;
            }
            ...
            for(int i = 0; i < 2000; ++i) {
               ...
               trajectoryX *= 0.999D;
               trajectoryY = trajectoryY * 0.999D;
               trajectoryZ *= 0.999D;
