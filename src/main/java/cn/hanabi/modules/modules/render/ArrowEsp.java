...
public class ArrowEsp extends Mod {
   private int alpha;
   private final Value size = new Value("ArrowEsp", "Size", 10.0D, 5.0D, 25.0D, 0.1D);
   private final Value radius = new Value("ArrowEsp", "Radius", 45.0D, 10.0D, 200.0D, 1.0D);
   ...
   private boolean isOnScreen(Vec3 pos) {
      if (pos.xCoord > -1.0D && pos.zCoord < 1.0D) {
         return pos.xCoord / (double)(mc.gameSettings.guiScale == 0 ? 1 : mc.gameSettings.guiScale) >= 0.0D && pos.xCoord / (double)(mc.gameSettings.guiScale == 0 ? 1 : mc.gameSettings.guiScale) <= (double)Display.getWidth() && pos.yCoord / (double)(mc.gameSettings.guiScale == 0 ? 1 : mc.gameSettings.guiScale) >= 0.0D && pos.yCoord / (double)(mc.gameSettings.guiScale == 0 ? 1 : mc.gameSettings.guiScale) <= (double)Display.getHeight();
      } else {
         return false;
      }
   }
   ...
   private boolean isValid(EntityLivingBase entity) {
      return !AntiBot.isBot(entity) && entity != mc.thePlayer && this.isValidType(entity) && entity.getEntityId() != -1488 && entity.isEntityAlive() && (!entity.isInvisible() || ((Boolean)this.invisibles.getValueState()).booleanValue());
   }

   private boolean isValidType(EntityLivingBase entity) {
      return ((Boolean)this.players.getValueState()).booleanValue() && entity instanceof EntityPlayer || ((Boolean)this.mobs.getValueState()).booleanValue() && (entity instanceof EntityMob || entity instanceof EntitySlime) || ((Boolean)this.passives.getValueState()).booleanValue() && (entity instanceof EntityVillager || entity instanceof EntityGolem) || ((Boolean)this.animals.getValueState()).booleanValue() && entity instanceof EntityAnimal;
   }
   ...
   public static class EntityListener {
      ...
      private void render3d() {
         ...
         for(Entity e : ArrowEsp.mc.theWorld.loadedEntityList) {
