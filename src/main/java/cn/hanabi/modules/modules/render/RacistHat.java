...
public class RacistHat extends Mod {
   ...
   public Value side = new Value("BambooHat", "Side", 45.0D, 30.0D, 50.0D, 1.0D);
   public Value stack = new Value("BambooHat", "Stacks", 50.0D, 45.0D, 200.0D, 5.0D);
   ...
   public void onRender3D(EventRender evt) {
      if (mc.gameSettings.thirdPersonView != 0 || ((Boolean)this.renderInFirstPerson.getValueState()).booleanValue()) {
   ...
   private void drawDoli(EntityLivingBase entity, EventRender evt) {
      ...
      GL11.glPushMatrix();
      GL11.glTranslated(x, y + (mc.thePlayer.isSneaking() ? 2.0D : 2.2D), z);
