...
public class HUD extends Mod {
   ...
   public static Value musicPosX = new Value("HUD", "MusicPlayerX", 70.0D, 0.0D, 400.0D, 1.0D);
   public static Value musicPosY = new Value("HUD", "MusicPlayerY", 5.0D, 0.0D, 200.0D, 1.0D);
   public static Value musicPosYlyr = new Value("HUD", "MusicPlayerLyricY", 120.0D, 0.0D, 200.0D, 1.0D);
   public static Value hudMode = new Value("HUD", "HudMode", 0);
   private final HashMap<Integer, Integer> potionMaxDurations = new HashMap<>();
   private final HUD.WaitTimer tpsTimer = new HUD.WaitTimer();
   ...
   public static Value rainbowspeed = new Value("HUD", "ArrayList Speed", 3.0D, 1.0D, 6.0D, 1.0D);
   public static Value offset = new Value("HUD", "RainBow Offset", 2.0D, 1.0D, 6.0D, 1.0D);
   public static Value fade = new Value("HUD", "Fade Offset", 14.0D, 1.0D, 20.0D, 1.0D);
   ...
   boolean isLag;
   private SmoothAnimation hotbarAnimation = new SmoothAnimation(0.0D, 200);
   ...
   public HUD() {
      ...
      for(Mod module : ModManager.getModules()) {
         ...
         ((List)moduleCategoryMap.get(module.getCategory())).add(module);
      }
   ...
   public static int rainbow(int delay) {
      ...
      rainbowState = rainbowState % 360.0D;
   ...
   public void onText(EventText event) {
      if (mc.thePlayer != null) {
         if (((Boolean)this.fixname.getValue()).booleanValue()) {
   ...
   private void render2D(EventRender2D event) {
      ...
      if (((Boolean)this.potion.getValueState()).booleanValue()) {
         this.renderPotionStatus((int)width, (int)height);
      }

      if (((Boolean)this.compass.getValueState()).booleanValue()) {
      ...
      if (((Boolean)this.arraylist.getValueState()).booleanValue()) {
         this.renderArray(sr);
      }

      if (((Boolean)this.noti.getValueState()).booleanValue()) {
      ...
      if (((Boolean)this.music.getValueState()).booleanValue()) {
         MusicOverlayRenderer.INSTANCE.renderOverlay();
      }

      if (((Boolean)this.logo.getValueState()).booleanValue()) {
      ...
      if (((Boolean)this.posDisplay.getValueState()).booleanValue()) {
         String pos = "";
         if (mc.thePlayer != null) {
      ...
      if (((Boolean)this.hotbar.getValueState()).booleanValue() && mc.getRenderViewEntity() instanceof EntityPlayer && !mc.gameSettings.hideGUI) {
         ...
         String ez = "Hanabi Build 3.30227 - " + Client.username;
   ...
   private void renderArray(ScaledResolution sr) {
      ...
      for(Mod module : mods) {
         ...
         if (array.isCurrentMode("Random")) {
            ...
            base = RenderUtil.getRainbow(6000, (int)(-15.0F * nextY), ((Double)rainbowspeed.getValue()).doubleValue(), ((Double)offset.getValue()).doubleValue(), design.saturation, design.brightness);
         }

         int color;
         if (((Boolean)this.arraylistfade.getValueState()).booleanValue()) {
            if (simple) {
         ...
         if (!module.getName().equals("TargetStrafe") && module.getCategory() != Category.RENDER && (module.isEnabled() || module.posX > 0.0F)) {
            ...
            first = false;
            lastX = (int)((float)sr.getScaledWidth() - modwidth - 3.0F);
            if (displayName != null) {
               if (simple) {
   ...
   public void renderBlockCount(float width, float height) {
      ...
      this.yAxisAnimation = RenderUtil.getAnimationState(this.yAxisAnimation, state ? 0.0F : 10.0F, (float)Math.max(10.0D, (double)(Math.abs(this.yAxisAnimation - (float)(state ? 0 : 10)) * 50.0F) * 0.5D));
   ...
   public void drawArrowRect(float left, float top, float right, float bottom, int color) {
      ...
      bufferBuilder.pos((double)(left - 5.0F), (double)bottom, 0.0D).endVertex();
      bufferBuilder.pos((double)(right + 5.0F), (double)bottom, 0.0D).endVertex();
   ...
   public void renderPotionStatus(int width, int height) {
      ...
      for(Entry entry : this.potionMaxDurations.entrySet()) {
         if (mc.thePlayer.getActivePotionEffect(Potion.potionTypes[((Integer)entry.getKey()).intValue()]) == null) {
            needRemove.add(entry.getKey());
         }
      }

      Iterator var19 = needRemove.iterator();

      while(var19.hasNext()) {
         int id = ((Integer)var19.next()).intValue();
         this.potionMaxDurations.remove(Integer.valueOf(id));
      }

      for(PotionEffect effect : mc.thePlayer.getActivePotionEffects()) {
         if (!this.potionMaxDurations.containsKey(Integer.valueOf(effect.getPotionID())) || ((Integer)this.potionMaxDurations.get(Integer.valueOf(effect.getPotionID()))).intValue() < effect.getDuration()) {
         ...
         float rectX = (float)(width - 120) + 110.0F * ((float)effect.getDuration() / (1.0F * (float)((Integer)this.potionMaxDurations.get(Integer.valueOf(effect.getPotionID()))).intValue()));
   ...
   public void onRPacket(EventPacket event) {
      if (event.getPacket() instanceof S03PacketTimeUpdate) {
         times.add(Long.valueOf(Math.max(1000L, this.tpsTimer.getTime())));
         long timesAdded = 0L;
         if (times.size() > 5) {
            times.remove(0);
         }
         ...
         long l;
         for(Iterator roundedTps = times.iterator(); roundedTps.hasNext(); timesAdded += l) {
            l = ((Long)roundedTps.next()).longValue();
         }

         long roundedTps = timesAdded / (long)times.size();
         this.lastTPS = (float)(20.0D / (double)roundedTps * 1000.0D);
         this.tpsTimer.reset();
      }
