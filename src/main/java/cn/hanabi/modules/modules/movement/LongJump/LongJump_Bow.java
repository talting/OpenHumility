...
public class LongJump_Bow {
   ...
   public void onPre(EventPreMotion e) {
      if (!this.jumped && !this.mc.thePlayer.onGround && this.mc.thePlayer.motionY > 0.0D) {
         this.jumped = true;
      }
   ...
   public void onMove(EventMove event) {
      if (!this.dmged && this.stage > 0) {
         PlayerUtil.setSpeed(event, 0.0D);
      } else {
         if (this.stage == 4) {
            event.setY(this.mc.thePlayer.motionY = PlayerUtil.getBaseJumpHeight() * 1.9500000476837158D);
            this.speed = PlayerUtil.getBaseMoveSpeed() * 1.3D;
         } else if (this.stage >= 4 && this.stage != 5) {
            if (!this.mc.theWorld.getCollidingBoundingBoxes(this.mc.thePlayer, this.mc.thePlayer.getEntityBoundingBox().offset(0.0D, this.mc.thePlayer.motionY, 0.0D)).isEmpty() || this.mc.thePlayer.isCollidedVertically && this.mc.thePlayer.onGround) {
               ModManager.getModule("LongJump").set(false);
            }

            event.setY(this.mc.thePlayer.motionY += 0.004999999888241291D - this.speed / PlayerUtil.getBaseMoveSpeed() * 0.004999999888241291D);
            this.speed = this.last - this.last / 159.0D;
         }
