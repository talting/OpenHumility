...
public class RotationUtil {
   ...
   public static float[] getRotationToLocation(Vec3 loc) {
      ...
      float pitch = (float)(-(Math.atan2(yDiff, distance) * 180.0D / 3.141592653589793D));
   ...
   public static double getRotationDifference(Rotation rotation) {
      return prevRotations == null ? 0.0D : getRotationDifference(rotation, new Rotation(prevRotations[0], prevRotations[1]));
   }

   public static float[] rotations(Entity target) {
      double x = target.posX - mc.thePlayer.posX;
      double z = target.posZ - mc.thePlayer.posZ;
      double y = target.posY + (double)target.getEyeHeight() * 0.75D - (mc.thePlayer.posY + (double)mc.thePlayer.getEyeHeight());
   ...
   public static float[] getNeededRotations(Vec3 vec, Vec3 transected) {
      boolean predict = ((Boolean)KillAura.predict.getValue()).booleanValue();
      Vec3 targetVector = new Vec3(vec.xCoord + (predict ? transected.xCoord : 0.0D), vec.yCoord + (predict ? transected.yCoord : 0.0D), vec.zCoord + (predict ? transected.zCoord : 0.0D));
      Vec3 playerVector = new Vec3(mc.thePlayer.posX + mc.thePlayer.motionX * ((Double)KillAura.predictor.getValue()).doubleValue() * (double)RandomUtils.nextFloat(0.8F, 1.2F), mc.thePlayer.getEntityBoundingBox().minY + (double)mc.thePlayer.getEyeHeight() + mc.thePlayer.motionY, mc.thePlayer.posZ + mc.thePlayer.motionZ * ((Double)KillAura.predictor.getValue()).doubleValue() * (double)RandomUtils.nextFloat(0.8F, 1.2F));
   ...
   public static float[] getRotations(double posX, double posY, double posZ, double eyeHeight, BlockPos blockPos, EnumFacing enumFacing) {
      double n = (double)blockPos.getX() + 0.5D - posX + (double)enumFacing.getFrontOffsetX() / 2.0D;
      double n2 = (double)blockPos.getZ() + 0.5D - posZ + (double)enumFacing.getFrontOffsetZ() / 2.0D;
      double n3 = posY + eyeHeight - ((double)blockPos.getY() + 0.5D);
      double n4 = (double)MathHelper.sqrt_double(n * n + n2 * n2);
      float n5 = (float)(Math.atan2(n2, n) * 180.0D / 3.141592653589793D) - 90.0F;
      float n6 = (float)(Math.atan2(n3, n4) * 180.0D / 3.141592653589793D);
      if (n5 < 0.0F) {
         n5 += 360.0F;
      }
