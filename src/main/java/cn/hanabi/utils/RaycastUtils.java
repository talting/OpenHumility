...
public final class RaycastUtils {
   ...
   public static Entity raycastEntity(double range, Rotation rotation, RaycastUtils.IEntityFilter entityFilter) {
      return raycastEntity(range, rotation.getYaw(), rotation.getPitch(), entityFilter);
   }

   private static Entity raycastEntity(double range, float yaw, float pitch, RaycastUtils.IEntityFilter entityFilter) {
      ...
      if (renderViewEntity != null && mc.theWorld != null) {
         ...
         List<Entity> entityList = mc.theWorld.getEntitiesInAABBexcluding(renderViewEntity, renderViewEntity.getEntityBoundingBox().addCoord(entityLook.xCoord * range, entityLook.yCoord * range, entityLook.zCoord * range).expand(1.0D, 1.0D, 1.0D), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBeCollidedWith));
