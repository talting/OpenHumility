...
public class MathUtils {
   ...
   public static Double[] lerp(Double[] a, Double[] b, double t) {
      return new Double[]{a[0].doubleValue() + (b[0].doubleValue() - a[0].doubleValue()) * t, a[1].doubleValue() + (b[1].doubleValue() - a[1].doubleValue()) * t};
   }

   public static double distanceSq(Double[] a, Double[] b) {
      return Math.pow(a[0].doubleValue() - b[0].doubleValue(), 2.0D) + Math.pow(a[1].doubleValue() - b[1].doubleValue(), 2.0D);
   }

   public static double distanceToSegmentSq(Double[] p, Double[] v, Double[] w) {
      double l2 = distanceSq(v, w);
      return l2 == 0.0D ? distanceSq(p, v) : distanceSq(p, lerp(v, w, GLUtil.glmClamp(((p[0].doubleValue() - v[0].doubleValue()) * (w[0].doubleValue() - v[0].doubleValue()) + (p[1].doubleValue() - v[1].doubleValue()) * (w[1].doubleValue() - v[1].doubleValue())) / l2, 0.0D, 1.0D)));
   }
   ...
   public static Double[][] getPointsOnCurve(Double[][] points, int num) {
      ...
      for(int i = 0; i < num; ++i) {
         double t = (double)i / ((double)num - 1.0D);
