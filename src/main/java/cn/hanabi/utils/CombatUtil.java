...
public class CombatUtil {
   ...
   public static float[] getRotationNeededForBlock(EntityPlayer paramEntityPlayer, BlockPos pos) {
      ...
      double d2 = (double)pos.getY() + 0.5D - (paramEntityPlayer.posY + (double)paramEntityPlayer.getEyeHeight());
   ...
   public static float getYaw(Entity var0) {
      ...
      double var5 = var3 < 0.0D && var1 < 0.0D ? 90.0D + Math.toDegrees(Math.atan(var3 / var1)) : (var3 < 0.0D && var1 > 0.0D ? -90.0D + Math.toDegrees(Math.atan(var3 / var1)) : Math.toDegrees(-Math.atan(var1 / var3)));
