...
public class AStarCustomPathFinder {
   private static final Vec3[] flatCardinalDirections = new Vec3[]{new Vec3(1.0D, 0.0D, 0.0D), new Vec3(-1.0D, 0.0D, 0.0D), new Vec3(0.0D, 0.0D, 1.0D), new Vec3(0.0D, 0.0D, -1.0D)};
   ...
   public void compute(int loops, int depth) {
      ...
      this.hubsToWork.add(new AStarCustomPathFinder.Hub(this.startVec3, (AStarCustomPathFinder.Hub)null, initPath, this.startVec3.squareDistanceTo(this.endVec3), 0.0D, 0.0D));
      ...
      for(int i = 0; i < loops; ++i) {
         ...
         for(AStarCustomPathFinder.Hub hub : new ArrayList<Hub>(this.hubsToWork)) {
            ++j;
            if (j > depth) {
               break;
            }
   ...
   public AStarCustomPathFinder.Hub isHubExisting(Vec3 loc) {
      ...
      for(AStarCustomPathFinder.Hub hub : this.hubsToWork) {
         if (hub.getLoc().getX() == loc.getX() && hub.getLoc().getY() == loc.getY() && hub.getLoc().getZ() == loc.getZ()) {
            return hub;
         }
      }
   ...
   public boolean addHub(AStarCustomPathFinder.Hub parent, Vec3 loc, double cost) {
      ...
      if (existingHub == null) {
         ...
         if (loc.getX() == this.endVec3.getX() && loc.getY() == this.endVec3.getY() && loc.getZ() == this.endVec3.getZ() || minDistanceSquared != 0.0D && loc.squareDistanceTo(this.endVec3) <= minDistanceSquared) {
            this.path.clear();
   ...
   private static class Hub {
      ...
      public Hub(Vec3 loc, AStarCustomPathFinder.Hub parent, ArrayList path, double squareDistanceToFromTarget, double cost, double totalCost) {
         super();
         this.loc = loc;
         this.parent = parent;
         this.path = path;
