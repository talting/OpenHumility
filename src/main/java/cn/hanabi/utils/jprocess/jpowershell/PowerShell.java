...
public class PowerShell implements AutoCloseable {
   ...
   public PowerShell configuration(Map config) {
      try {
         this.waitPause = Integer.valueOf(config != null && config.get("waitPause") != null ? (String)config.get("waitPause") : PowerShellConfig.getConfig().getProperty("waitPause")).intValue();
         this.maxWait = Long.valueOf(config != null && config.get("maxWait") != null ? (String)config.get("maxWait") : PowerShellConfig.getConfig().getProperty("maxWait")).longValue();
   ...
   public boolean isLastCommandInError() {
      return !Boolean.valueOf(this.executeCommand("$?").getCommandOutput()).booleanValue();
   }
   ...
   private File createWriteTempFile(BufferedReader srcReader) {
      ...
      try {
         ...
         if (tmpFile.exists()) {
            ...
            while(srcReader != null && (ex = srcReader.readLine()) != null) {
               tmpWriter.write(ex);
               tmpWriter.newLine();
            }
   ...
   private long getPID() {
      ...
      return !commandOutput.isEmpty() ? Long.valueOf(commandOutput).longValue() : -1L;
   }
